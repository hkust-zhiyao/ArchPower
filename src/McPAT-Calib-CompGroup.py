...
def draw_figure(gt,pd,name):
    ...
    plt.savefig("../figure/{}_comp_group.png".format(name),dpi=200)
    plt.close()
    return
...
def known_n_config(training_index, testing_index, uarch, fname):
    ...
    train_feature, group0_train_label_total, test_feature, test_label_total = generate_training_test(feature, label_total, training_index, testing_index)

    feature, group0_label_total = load_data(uarch, 1)
    feature, group1_label_total = load_data(uarch, 2)
    feature, group2_label_total = load_data(uarch, 3)
    feature, group3_label_total = load_data(uarch, 4)

    train_feature, group0_train_label_total, test_feature, group0_test_label_total = generate_training_test(feature, group0_label_total, training_index, testing_index)
    train_feature, group1_train_label_total, test_feature, group1_test_label_total = generate_training_test(feature, group1_label_total, training_index, testing_index)
    train_feature, group2_train_label_total, test_feature, group2_test_label_total = generate_training_test(feature, group2_label_total, training_index, testing_index)
    train_feature, group3_train_label_total, test_feature, group3_test_label_total = generate_training_test(feature, group3_label_total, training_index, testing_index)

    # print(train_feature.shape,train_label_total.shape, test_feature.shape, test_label_total.shape)
    # exit()

    group0_model = build_logic_model(train_feature, group0_train_label_total)
    group1_model = build_logic_model(train_feature, group1_train_label_total)
    group2_model = build_logic_model(train_feature, group2_train_label_total)
    group3_model = build_logic_model(train_feature, group3_train_label_total)

    group0_pred = test_logic_model(test_feature, test_label_total, group0_model)
    group1_pred = test_logic_model(test_feature, test_label_total, group1_model)
    group2_pred = test_logic_model(test_feature, test_label_total, group2_model)
    group3_pred = test_logic_model(test_feature, test_label_total, group3_model)

    pred = group0_pred + group1_pred + group2_pred + group3_pred

    for i in range(12):
        draw_figure(test_label_total[:,i],pred[:,i],"McPAT_Calib_CompGroup/{}_{}_".format(uarch,fname)+figure_name[i])
