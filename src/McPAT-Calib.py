...
component_model_selection=[0,0,2,2,2,2,2,2,0,0,
                            0,0,2,0,0,2,0,1,1,1,
                            0,0,0]
...
def draw_figure(gt,pd,name):
    ...
    plt.savefig("../figure/{}_calib.png".format(name),dpi=200)
    plt.close()
    return
...
def build_logic_model(feature,label):
    ...
    # build model
    # this_model = GradientBoostingRegressor()
    this_model = xgb.XGBRegressor()
    this_model.fit(feature[:,feature_index],label)

    # this_model.save_model('xgb/{}.json'.format(dump_name))

    return this_model

def test_logic_model(feature,label,this_model):
    component = "Others"
...
def known_n_config(training_index, testing_index, uarch, fname):
    ...
    logic_model = build_logic_model(train_feature, train_label_total[:,0])
    logic_pred = test_logic_model(test_feature, test_label_total[:,0], logic_model)

    draw_figure(test_label_total[:,0],logic_pred,"McPAT_Calib/{}_{}_".format(uarch,fname)+figure_name[0])
